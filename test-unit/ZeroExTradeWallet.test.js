...
describe("Test ZeroExTradeWallet", () => {
	...
	describe("Signer Authorization", () => {
		...
		it("Can successfully call the router", async () => {
			await expect(tradeWallet.connect(deployer).registerAllowedOrderSigner(user1.address, true)).to.not.be
				.reverted;
			await expect(tradeWallet.connect(deployer).registerAllowedOrderSigner(user1.address, false)).to.not.be
				.reverted;
		});
	});
});
