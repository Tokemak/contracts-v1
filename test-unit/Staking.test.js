...
describe("Test Staking", () => {
	...
	before(async () => {
		...
		stakingContract = await upgrades.deployProxy(
			...
			[underlyingToken.address, managerContract.address, treasuryAddress, notionalAddress1.address],
			{
				unsafeAllow: ["constructor"],
			}
		);
		await stakingContract.deployed();
		stakingContract.connect(deployer).setAccToke(accToke.address);
	});
	...
	describe("Test withdraw and migrate to accToke", () => {
		beforeEach(async () => {
			await underlyingToken.mock.transferFrom.returns(true);
			...
			await accToke.mock.lockTokeFor.returns();
		});

		it("Reverts on invalid number of cycle", async () => {
			await expect(stakingContract.connect(user1).withdrawAndMigrate(10, 0)).to.be.revertedWith(
				"INVALID_NB_CYCLES"
			);
		});

		it("Reverts on invalid amount", async () => {
			await expect(stakingContract.connect(user1).withdrawAndMigrate(0, 0)).to.be.revertedWith("INVALID_AMOUNT");
		});

		it("Reverts on insufficient user balance", async () => {
			await expect(stakingContract.connect(user1).withdrawAndMigrate(1, 1)).to.be.revertedWith(
				"INSUFFICIENT_AVAILABLE"
			);
		});

		it("Reverts there is transfer queued", async () => {
			await stakingContract.connect(user1)["deposit(uint256,uint256)"](10, 0);

			await stakingContract.connect(user1).queueTransfer(0, 0, 3, user2.address);

			await expect(stakingContract.connect(user1).withdrawAndMigrate(10, 1)).to.be.revertedWith(
				"CANT_HAVE_TRANSFER_QUEUED"
			);
		});

		it("Will only update withdrawn", async () => {
			const deposit = 10;
			const toRequestWithdraw = 6;
			const toWithdrawAndMigrate = 4;

			// We want to withdraw and migrate "4" to accToke
			// Available balance to withdraw is "4" (10 - 6)
			// withdrawalRequests (toRequestWithdraw) should remain untouched

			await stakingContract.connect(user1)["deposit(uint256,uint256)"](deposit, 0);

			await stakingContract.connect(user1).requestWithdrawal(toRequestWithdraw, 0);

			await stakingContract.connect(user1).withdrawAndMigrate(toWithdrawAndMigrate, 1);

			const withheld = await stakingContract.connect(user1).withdrawalRequestsByIndex(user1.address, 0);
			const stake = await stakingContract.userStakings(user1.address, 0);

			expect(stake.withdrawn).to.equal(toWithdrawAndMigrate);
			expect(toRequestWithdraw).to.equal(withheld.amount);
		});

		it("Will update withdrawn and withdrawalRequests", async () => {
			const deposit = 10;
			const toRequestWithdraw = 5;
			const toWithdrawAndMigrate = 7;

			// We want to withdraw and migrate "7" to accToke
			// Available balance to withdraw is "5" (10 - 5)
			// To withdraw "7", we have to take 2 from the withdrawalRequests
			// Removing "2" from withdrawalRequests (toRequestWithdraw) will leave "3" (5 - 2) to withdrawalRequests
			// setting expectedWithHeld to "3"

			const expectedWithHeld = 3;
			...
			expect(stake.withdrawn).to.equal(toWithdrawAndMigrate);
			expect(expectedWithHeld).to.equal(withheld.amount);
		});
	});
});
