...
describe("Test CoreEvent", () => {
	...
	describe("Test deposit()", () => {
		...
		it("Reverts when second deposit of singular asset exceeds limit", async () => {
			await expect(coreContract.connect(user1).deposit([{ token: mockToken1.address, amount: 5000 }], [])).to.not
				.be.reverted;
	...
	describe("Test withdraw()", async () => {
		...
		it("Reverts on zero address", async () => {
			await expect(
				coreContract.connect(user1).withdraw([{ token: ADDRESS_ZERO, amount: 20 }])
			).to.be.revertedWith("ZERO_ADDRESS");
		});
