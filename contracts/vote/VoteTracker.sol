...
contract VoteTracker is Initializable, EventReceiver, IVoteTracker, Ownable, Pausable {
	...
	function vote(UserVotePayload memory userVotePayload, Signature memory signature) external override whenNotPaused {
		...
		if (proxySubmitters[msg.sender]) {
			require(
				lastUserProxyVoteBlock[userVotePayload.account].add(settings.voteEveryBlockLimit) < block.number,
				"TOO_FREQUENT_VOTING"
			);
		...
		address signatureSigner = _hash(domainChain, userVotePayload, signature.signatureType).recover(
			signature.v,
			signature.r,
			signature.s
		);
	...
	function updateUserVoteTotals(address[] memory accounts) public override {
		for (uint256 i = 0; i < accounts.length; ++i) {
			...
			uint256 maxVotesToUse = Math.min(maxAvailableVotes, userVoteDetails[account].totalUsedVotes);
			...
			if (maxVotesToUse > 0) {
				for (uint256 j = 0; j < keys.length; ++j) {
					...
					systemAggregations[placement.reactorKey] = systemAggregations[placement.reactorKey].add(
						placement.amount
					);
				}
			} else {
	...
	function getUserVotes(address account) public view override returns (UserVotes memory) {
		...
		for (uint256 i = 0; i < keys.length; ++i) {
			placements[i] = UserVoteAllocationItem({ reactorKey: keys[i], amount: userVoteItems[account][keys[i]] });
		}
	...
	function getSystemVotes() public view override returns (SystemVotes memory systemVotes) {
		...
		systemVotes = SystemVotes({
			details: SystemVoteDetails({ voteSessionKey: settings.voteSessionKey, totalVotes: totalVotes }),
			votes: votes
		});
	}
	...
	function getVotingPower(TokenBalance[] memory balances) external view override returns (uint256 votes) {
	...
	function _onEventReceive(address, bytes32 eventType, bytes calldata data) internal virtual override {
		...
		for (uint256 i = 0; i < userVotePayload.allocations.length; ++i) {
			...
			if (userVoteItems[account][reactorKey] > 0) {
				...
				if (currentAmount > amount) {
					systemAggregations[reactorKey] = systemAggregations[reactorKey].sub(currentAmount - amount);
					totalUsedVotes = totalUsedVotes.sub(currentAmount - amount);
				} else if (currentAmount < amount) {
					systemAggregations[reactorKey] = systemAggregations[reactorKey].add(amount - currentAmount);
	...
	function _getVotingPower(TokenBalance[] memory balances) private view returns (uint256 votes) {
		for (uint256 i = 0; i < balances.length; ++i) {
			votes = votes.add(
				balances[i].amount.mul(voteMultipliers[balances[i].token]).div(ONE_WITH_EIGHTEEN_PRECISION)
			);
		}
	}
	...
	function _buildDomainSeparator(uint256 chainId) private view returns (bytes32) {
		return keccak256(abi.encode(EIP712_DOMAIN_TYPEHASH, DOMAIN_NAME, DOMAIN_VERSION, chainId, address(this)));
	}
	...
	function _hash(
		...
		bytes32 x = keccak256(
			abi.encodePacked(EIP191_HEADER, _domainSeparatorV4(domainChain), _hashUserVotePayload(userVotePayload))
		);
	...
	function _hashUserVoteAllocationItem(UserVoteAllocationItem memory voteAllocation) private view returns (bytes32) {
		return
			keccak256(abi.encode(USER_VOTE_ALLOCATION_ITEM_TYPEHASH, voteAllocation.reactorKey, voteAllocation.amount));
	}
}
