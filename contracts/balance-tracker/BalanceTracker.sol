...
contract BalanceTracker is EventReceiver, IBalanceTracker, Ownable {
	...
	) external view override returns (TokenBalance[] memory userBalances) {
	...
	function setBalance(SetTokenBalance[] calldata balances) external override onlyOwner {
		for (uint256 i = 0; i < balances.length; ++i) {
			...
			updateBalance({ account: balance.account, token: balance.token, amount: balance.amount, stateSync: false });
		}
	}
	...
	function updateBalance(address account, address token, uint256 amount, bool stateSync) private {
		...
		if (stateSync || userTokenBalance.token == address(0)) {
			...
			if (delegatedTo[account] != address(0)) {
				...
				delegatedBalance[delegatedTo[account]][token] = delegatedBalance[delegatedTo[account]][token]
					.sub(userTokenBalance.amount)
					.add(amount);
			}

			// Add the balance delegated to the account (since the amount in the event does not include delegated balance)
			accountTokenBalances[account][token].amount = accountTokenBalances[account][token].amount.add(
				delegatedBalance[account][token]
			);

			//Update the total based on the individual amounts
			_updateTotalTokenBalance(token, userTokenBalance.amount, accountTokenBalances[account][token].amount);
	...
	function _delegate(address token, address from, address to) private {
		...
		if (delegatedTo[from] != address(0)) {
			...
			delegatedBalance[delegatedTo[from]][token] = delegatedBalance[delegatedTo[from]][token].sub(
	...
	function _delegateAll(address from, address to, bytes32 functionId) private {
	...
	function getSupportedTokens() external view override returns (address[] memory supportedTokensArray) {
	...
	function _updateTotalTokenBalance(address token, uint256 oldAmount, uint256 newBalance) private {
