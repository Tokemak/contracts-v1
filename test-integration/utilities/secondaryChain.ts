...
export class SecondaryChain {
	...
	constructor(
		...
		protected startupTimeLimitSeconds: number = process.env.IS_CI === "true" ? 120 : 20
	) {}
	...
	private startupProcess() {
		...
		const startupByExpiration = new Date();
		startupByExpiration.setSeconds(startupByExpiration.getSeconds() + this.startupTimeLimitSeconds);
	...
	private async checkStarted(
		endpoint: string,
		startupByExpiration: Date,
		finishSetup: (value: unknown) => void,
		failSetup: (reason: string) => void,
		proc: ChildProcess
	) {
		if (new Date() > startupByExpiration) {
			failSetup("Did not startup by expiration");
			return;
		}
		...
		try {
			...
			setTimeout(() => {
				this.checkStarted(endpoint, startupByExpiration, finishSetup, failSetup, proc);
			}, 1000);
		}
	}
}
