...
import { Environment, getWhaleAddress, Contract, getContractAddress } from "utils/config";
...
describe("Test CoreEvent", () => {
	...
	const AAVE_ADDRESS = getContractAddress(Environment.MAINNET, Contract.AAVE);
	const LINK_ADDRESS = getContractAddress(Environment.MAINNET, Contract.LINK);
	const SUSHI_ADDRESS = getContractAddress(Environment.MAINNET, Contract.SUSHI);

	const AAVE_WHALE = getWhaleAddress(Environment.MAINNET, Contract.AAVE);
	const LINK_WHALE = getWhaleAddress(Environment.MAINNET, Contract.LINK);
	const SUSHI_WHALE = getWhaleAddress(Environment.MAINNET, Contract.SUSHI);
		...
		it("Forces funds to be withdrawn and sends to farm based on rates", async () => {
			...
			await expect(core.connect(linkWhale).finalize([{ token: LINK_ADDRESS, sendToFarming: true }]))
				...
				.withManuallyValidatedArgs<CoreEvent>(core, (args: any[]) => {
					...
					expect(args[1][0].token.toString()).to.equal(LINK_ADDRESS);
			...
			await expect(core.connect(sushiWhale).finalize([{ token: SUSHI_ADDRESS, sendToFarming: false }]))
				...
				.withManuallyValidatedArgs<CoreEvent>(core, (args: any[]) => {
					...
					expect(args[1][0].token.toString()).to.equal(SUSHI_ADDRESS);
		...
		it("An early transfer to treasury still allows appropriate withdraw by users", async () => {
			...
			await expect(core.connect(aaveWhale).finalize([{ token: AAVE_ADDRESS, sendToFarming: false }]))
				...
				.withManuallyValidatedArgs<CoreEvent>(core, (args: any[]) => {
					...
					expect(args[1][0].token.toString()).to.equal(AAVE_ADDRESS);
