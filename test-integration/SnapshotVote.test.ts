...
import { Environment, getContractAddress, Contract } from "utils/config";
...
describe("Snapshot Toke", () => {
	...
	before(async () => {
		...
		deployer = await getImpersonatedSigner(getContractAddress(Environment.MAINNET, Contract.DEPLOYER));

		const ENV = Environment.MAINNET;
		const stakingAddress = getContractAddress(ENV, Contract.STAKING);
		const tokeAddress = getContractAddress(ENV, Contract.TOKE);
		const delegateAddress = getContractAddress(ENV, Contract.DELEGATE);
		...
		snapshotToke = await snapshotTokeFactory.deploy(
			getContractAddress(ENV, Contract.SUSHI_POOL),
			stakingAddress,
			delegateAddress,
			tokeAddress
		);
