...
const TOKE_POOL_PROXY = getContractAddress(Environment.MAINNET, Contract.TOKE_POOL);
const ETH_POOL_PROXY = getContractAddress(Environment.MAINNET, Contract.WETH_POOL);
...
describe("Test L1 Voting", () => {
	...
	before(async () => {
		...
		accToke = await deployMockContract(destination, IAccToke.abi);
		...
		const daoSigner = await getImpersonatedSigner(getContractAddress(Environment.MAINNET, Contract.TREASURY));
