...
const USDC_ADDRESS = getAddress(Environment.MAINNET, Contract.USDC);
const WETH_ADDRESS = getAddress(Environment.MAINNET, Contract.WETH);

const MANAGER = getAddress(Environment.MAINNET, Contract.MANAGER);
...
describe("Pool Transfer Controller", () => {
	before(async () => {
		...
		registry = (await upgrades.deployProxy(registryFactory, [], {
			constructorArgs: [getAddress(Environment.MAINNET, Contract.WETH)],
