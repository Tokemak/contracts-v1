...
import { getContractAddress as getAddress, Environment, Contract, getWhaleAddress } from "utils/config";
...
describe("Test Sushiswap Controller V2", () => {
	const SUSHISWAP_FACTORY_ADDRESS = getAddress(Environment.MAINNET, Contract.SUSHI_FACTORY);
	const SUSHISWAP_ROUTER_ADDRESS = getAddress(Environment.MAINNET, Contract.SUSHI_ROUTER);
	const MASTERCHEF_V2 = getAddress(Environment.MAINNET, Contract.MASTERCHEF_V2);

	const ALCX_WHALE = getWhaleAddress(Environment.MAINNET, Contract.ALCX);
	const WETH_WHALE = getWhaleAddress(Environment.MAINNET, Contract.WETH);

	const TOKEMAK_TREASURY = getAddress(Environment.MAINNET, Contract.TREASURY);

	const masterchefPoolArrayLength = 46;

	const alcxToken = getAddress(Environment.MAINNET, Contract.ALCX);
	const wethToken = getAddress(Environment.MAINNET, Contract.WETH);
	...
	describe("Rewarder DNE", () => {
		...
		const rulerToken = getAddress(Environment.MAINNET, Contract.RULER);
		const RULER_WHALE = getWhaleAddress(Environment.MAINNET, Contract.RULER);
