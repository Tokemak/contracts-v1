...
const getConfig = (env: Environment, addressKey: string) => {
	if (addressKey == Contract.VOTE_TRACKER_CORE) {
		return [
			{
				token: getContractAddress(env, Contract.TOKE_POOL),
				multiplier: ethers.utils.parseEther("6"),
			},
			{
				token: getContractAddress(env, Contract.UNI_POOL),
				multiplier: ethers.utils.parseEther("69"),
			},
			{
				token: getContractAddress(env, Contract.SUSHI_POOL),
				multiplier: ethers.utils.parseEther("69"),
			},
			{
				token: getContractAddress(env, Contract.STAKING),
