...
const getPoolBalanceDestinations = (env: Environment) => {
	return [
		getContractAddress(env, Contract.BALANCE_TRACKER),
		getContractAddress(env, Contract.VOTE_TRACKER_LD),
		getContractAddress(env, Contract.CORE3_VOTE_TRACKER),
	];
};
...
const getConfig = (env: Environment) => {
	...
	return [
		{
			sender: getContractAddress(env, Contract.TOKE_POOL),
			events: POOL_BALANCE_EVENTS,
			destinations: poolDestinations,
		},
		{
			sender: getContractAddress(env, Contract.UNI_POOL),
			events: POOL_BALANCE_EVENTS,
			destinations: poolDestinations,
		},
		{
			sender: getContractAddress(env, Contract.SUSHI_POOL),
			events: POOL_BALANCE_EVENTS,
			destinations: poolDestinations,
		},
		{
			sender: getContractAddress(env, Contract.STAKING),
			events: POOL_BALANCE_EVENTS,
			destinations: poolDestinations,
		},
		{
		...
		{
			sender: getContractAddress(env, Contract.ON_CHAIN_VOTE_L1_CORE),
			events: [EventType.VOTE],
			destinations: [getContractAddress(env, Contract.VOTE_TRACKER_CORE)],
		},
		{
			sender: getContractAddress(env, Contract.ON_CHAIN_VOTE_L1_LD),
			events: [EventType.VOTE],
			destinations: [getContractAddress(env, Contract.VOTE_TRACKER_LD)],
		},
		{
			sender: getContractAddress(env, Contract.CORE3_ON_CHAIN_VOTE),
			events: [EventType.VOTE],
			destinations: [getContractAddress(env, Contract.CORE3_VOTE_TRACKER)],
		},
	];
};
