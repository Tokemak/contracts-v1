...
const WETH_ADDRESS = getContractAddress(Environment.GOERLI, Contract.WETH);
const USDC_ADDRESS = getContractAddress(Environment.GOERLI, Contract.USDC);

async function main() {
	//We're going to fork our mainnet environment and
	//set it up for a test scenario
	const [deployer] = await ethers.getSigners();
	const weth = getContractAddress(Environment.GOERLI, Contract.WETH);
