...
import { Environment, Contract, getContractAddress } from "utils/config";
...
async function rewardsDrainHandler(args: yargs.Arguments): Promise<void> {
	...
	const rewardsAddress = getContractAddress(env, Contract.REWARDS);
	const managerAddress = getContractAddress(env, Contract.MANAGER);
	const tokeAddress = getContractAddress(env, Contract.TOKE);
	const treasuryAddress = getContractAddress(env, Contract.TREASURY);
